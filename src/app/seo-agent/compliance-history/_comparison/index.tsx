...
const ComparisonRow = ({ label, expected, actual, status, message }: any) => {
  ...
  const expArr = isArrayData ? (Array.isArray(expected) ? expected.filter(Boolean) : []) : [];
  const actArr = isArrayData ? (Array.isArray(actual) ? actual.filter(Boolean) : []) : [];

  const expNorms = expArr.map(normalize).filter(Boolean);
  const actNorms = actArr.map(normalize).filter(Boolean);

  // Dynamically calculate actual matching items strictly for the UI rendering
  let matchedCount = 0;
  if (isArrayData) {
    expArr.forEach((item) => {
      const nItem = normalize(item);
      if (nItem && actNorms.some(a => a.includes(nItem) || nItem.includes(a))) {
        matchedCount++;
      }
    });
  }
  ...
  const renderArrayComparison = () => {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {expArr.length === 0 ? (
            ...
            <ul className="space-y-3">
              {expArr.map((item, i) => {
                ...
                const isFound = nItem && actNorms.some(a => a.includes(nItem) || nItem.includes(a));
        ...
        {/* ACTUAL COLUMN */}
        <div className={`rounded-xl border p-5 ${matchedCount === expArr.length ? 'bg-emerald-50/30 border-emerald-200' : 'bg-amber-50/30 border-amber-200'
          }`}
        >
          <div className="flex items-center gap-2 mb-4">
            <Globe className={`w-3.5 h-3.5 ${matchedCount === expArr.length ? 'text-emerald-500' : 'text-amber-500'}`} />
            <span className={`text-[10px] uppercase tracking-[0.2em] font-bold ${matchedCount === expArr.length ? 'text-emerald-600' : 'text-amber-600'
              }`}>
    ...
    <div className="group rounded-[24px] border border-neutral-200 bg-white p-6 shadow-sm hover:shadow-md transition-all">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-neutral-100 pb-4 mb-4 gap-2">
        <div className="flex items-center gap-3">
          <StatusIcon status={isArrayData ? (matchedCount === expArr.length ? 'pass' : 'warning') : status} />
          <h4 className="text-sm font-semibold uppercase tracking-widest text-neutral-900">{label}</h4>
        </div>

        {/* Dynamic Array Badge matching the design of the FAQ block */}
        {isArrayData ? (
          <span className={`text-[10px] uppercase tracking-widest font-semibold px-3 py-1 rounded-full ${matchedCount === expArr.length ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-700'
            }`}>
            Matched ({matchedCount}/{expArr.length})
          </span>
        ) : message && (
      ...
      {isArrayData ? renderArrayComparison() : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="rounded-xl bg-neutral-50 border border-neutral-200/60 p-5">
